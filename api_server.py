...
def check_key():
    try:
        ...
        # 👇 ЭТОТ БЛОК ЗАКОММЕНТИРОВАН - УБРАНА ПРОВЕРКА НА ОДНО УСТРОЙСТВО
        ...
        # Если ключ активируется впервые - просто обновляем last_used
        if not key_data['is_active']:
            cursor.execute('''
                UPDATE vless_keys 
                SET is_active = 1, activation_date = ?, last_used = ? 
                WHERE key_value = ?
            ''', (now.isoformat(), now.isoformat(), key))
            conn.commit()
            log_message(f"✅ Ключ {key} активирован")
        else:
